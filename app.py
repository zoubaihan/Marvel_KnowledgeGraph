...
from neo_db.query_graph import query, get_KGQA_answer, get_answer_profile, get_KGQA_answer_all
...
@app.route('/KGQA_answer_all', methods=['GET', 'POST'])
def KGQA_answer_all():
    question = request.args.get('name')
    json_data = get_KGQA_answer_all(get_target_array(str(question)))
    return jsonify(json_data)
